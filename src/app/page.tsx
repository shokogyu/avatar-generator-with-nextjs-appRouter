"use client";

import { ChangeEventHandler, MouseEventHandler, useCallback, useEffect, useMemo, useState } from "react";
import Image from "next/image";

// import { saveAs } from "file-saver";

import { DownloadButton } from "@/components/DownloadButton";

export default function Home() {
  return (
    <div className="space-y-10">
      <h1 className="text-center text-4xl font-black">Fun Avatar Generator</h1>
      <p className="text-md text-center leading-loose">
        このサイトでは、さまざまなスタイルのアバター画像を簡単に作成できます。
        <br />
        日々のアイコン設定にお悩みではありませんか？
        <br />
        SNSやゲーム、プロフィール画像など、日常のさまざまなシーンで使えるアイコンをお探しの方にぴったりです。
        <br />
        お気に入りのスタイルを見つけて、個性豊かなプロフィールを演出しましょう！
      </p>
      <p className="text-md text-center">
        Welcome to our site! Here, you can easily create various styles of avatar images. Are you struggling with your
        daily icon settings? Whether it's for social media, games, or profile pictures, we have a wide range of icons
        suitable for various everyday scenes. Find your favorite style and showcase your unique profile!
      </p>
    </div>
  );
}
